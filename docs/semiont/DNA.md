...
**#16 Peer 是 peer，不是 source material** — 把 peer 的 data 當線索（clue），從線索去搜 primary sources；不是把 peer 二手描述當 primary。操作規則見 [PEER-INGESTION-PIPELINE §6a](../pipelines/PEER-INGESTION-PIPELINE.md)。觸發：2026-04-12 ζ+ TFT P0 80% 骨架來自 peer，哲宇 1 分鐘抓到。

**#23 毒樹果實鏈 — 英文 summary → 具體細節腦補** — 錯誤四階段：source poisoning → inference poisoning → propagation → human cost。任何階段被汙染，下游全不可信。阻擋點：Stage 1 中文 prompt + verbatim / Stage 2 具體細節逐字對照中文原文 / Stage 3 SPORE Step 3c blocking gate。**memory 是自律，pipeline 才是閘門**。觸發：李洋孢子「清晨四點多搭捷運」[diary/2026-04-15-β.md](diary/2026-04-15-β.md)。
...
**#24 工具在說謊的三種形式** — 沉默失敗（silent skip）/ 合理欺騙（plausible wrong number）/ 通脹警報（alert inflation）。對策：警報 ≥ 100 件時，人工 sanity check 3-5 件。共同祖先是「規則在 A 定義算法在 B 複寫」（#17 程式碼層違反）。觸發：2026-04-15 β 同時發現 3 bug [memory/2026-04-15-β.md](memory/2026-04-15-β.md)。**延伸（2026-04-17 β）**：工具警報的單例不代表問題的集群——refresh-data 報 1 個 orphan 實際是 19 個同類（見 MEMORY §神經迴路 #65）。
...
**#15 反覆浮現的思考要儀器化，不能只寫原則** — 任何「每月/每週/每次心跳必做 X」承諾，沒 dashboard 欄位、cron 檢查、紅燈條件、escalation，都會三個月內被忘掉。**對自己的 bug 有洞察 ≠ apply 了 fix**。**memory 是自律，pipeline 才是閘門**。驗證 4 次：R6 每月原創（2026-04-11 ζ+）/ feedback_no_scene_inference（2026-04-15 β #29 放行）/ MANIFESTO §時間結構（2026-04-12 ζ+ 無收官自檢）/ Handoff 三態（2026-04-17 β，見 [diary/2026-04-17-β.md](diary/2026-04-17-β.md)）。
...
**#5 Pre-commit dogfood 是朋友不是敵人** — hook 阻擋 commit 是免疫正常。重新表述內容（把 PEM header 改成 regex 描述），不 `--no-verify` 繞過。
